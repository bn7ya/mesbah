...
onboarding, the Settings page, the theme, and the Simple/Expert UI mode.
...
- `service.py` — a tiny JSON store at `data/app_settings.json` (chmod 600; same
  ...
  - Accessors used elsewhere: `selected_gpu_indices()` (the canonical GPU choice;
    ...
    `is_onboarded()`, `ui_mode()`, `get_token(name)`.
...
(None ⇒ detected VRAM), `theme` (`light|dark`), `ui_mode` (`simple|expert`, default
`simple` — the frontend hides advanced/technical controls app-wide when `simple`;
`ui_mode()` normalizes any unexpected stored value back to `simple`), `tokens`
(generic `{name: secret}`; HF token keeps its own secure file in
